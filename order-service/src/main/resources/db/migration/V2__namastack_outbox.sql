-- Tables du transactional outbox Namastack 1.9 : DDL Postgres officiel (namastack-outbox-jdbc-1.9.0.jar,
-- schema/postgres/outbox-tables.sql), repris tel quel.
-- Idempotent (IF NOT EXISTS) : les bases créées avant cette migration ont déjà ces tables, générées au démarrage
-- par Namastack (namastack.outbox.jdbc.schema-initialization.enabled, désormais à false).
-- À la montée de version de Namastack : comparer avec le DDL du nouveau jar et ajouter une migration Vn.
